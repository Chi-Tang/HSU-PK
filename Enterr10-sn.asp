...
<% If Request("Send") = Empty Then %>
   <FORM Action=FormA-3.asp Method=POST>
   <INPUT Type=Hidden Name=Send Value="Send">
     <!--- 國曆年份：<INPUT Type= Text Name=Syerr Size=20 value=2012><P>
      ...
      <p>閏月：<select name="LLeap" size="1">                                                                                                                                                                                                                                                                                                                 
            <option value="false">否</option><option value="true">是</option> 
             </select>
          </p>--> 

           
      <h5> 農西元?年 　 &月份              &日號            &時辰                      &今年西元?年 :</h5>                                                                                                                                                                                                               
...
''offset = (Date.UTC(objDate.getFullYear(),objDate.getMonth(),objDate.getDate())-Date.UTC(1912,1,18))/86400000)
 ...
 ' Slunar=Lunar(Syern,Smohn,Sdatn)
  ...
  TermDatav =Array(0,21324,42505,63868,85407,107110,128977,151002,173218,195611,218134,240768,263418,286062,308631,331096,353423,375568,397546,419292,440895,462344,483626,504891,0,21324)
 Function ssTerm(y,nn) 
  ...
  ''DDt=DateDiff("d", Mn, D1t)
  Mn=( 525948.77*(y-1912) + TermDatav(nn))/1440 
   
   DDA=DateAdd("d",Mn,D1t)
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''
 ...
 DT22=DateSerial(KGyern,KGmohn,KGdatn )
 ''DYY=DateDiff("yyyy", D1, D2t)
  DDtD=DateDiff("d", DT11, DT22)
 Dim TLYMDS  , tyr,tmh,tsd,tslp
  Dim SDLunar,SDPLunar
  SDlunar=Lunar(Syern,Smohn,Sdatn)
    SDPLunar=split(SDlunar,";")
    tyr=SDPLunar(0)
    tmh=SDPLunar(1)
    tsd=SDPLunar(2)
    tslp=SDPLunar(3)
 '' Response.write SDLunar& SPLunar(3)
   '' Response.write SPLunar
    
 '' TLYMDS =  AcuYdsm(2012)
     TLYMDS =LYMDS(tyr,tmh,tsd,tslp)
            '''' GRNDSN(2012,10,10,"false") '' Lunar= thisyr&";"& thismth&";"&thisday&";"&thisLeap
     
  ''  Response.write "農總日"&(TLYMDS+47)&"<br>"
  ' Response.write "農總日"&(TLYMDS+47)&"<br>"
  ' Response.write "國總日"&DDtD
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''
 ...
 WHILE offsett >=0
  ...
  thisLeap = false
 '' '''''''''''''''''''''''''''''''''''''''''''''''''''''
  KLMDays=Array(0,31,29,31,30,31,30,31,31,30,31,30,31)
  KLMDayd=Array(0,31,28,31,30,31,30,31,31,30,31,30,31)
 ...
 '' KLnars=SECTM&"年;"&TNUM &"月;"&DNUM&"日;"
   ''  KLnar=offset&offsetm&offsetd
   '' KLnar=DD
 '' KLnar=KLnars&kisyr&"年"& kismth&"月"&kisday&"整總日"&DD
  '' KLnar= "<BR>"&ky&km&kd&klp&"農化國年"&kisyr&";"& kismth&";"&kisday&";"&kisLeap
 
  KLnar= "農化國年"&";"&kisyr&";"& kismth&";"&kisday&";"&kisLeap
 ...
 Dim DCA , DCD,DCLD 
  D1D=  #1912/1/7#  '''"節氣日"
  D2D= #1912/1/1#   '''"化國曆之起日"''D2L= #1912/2/18#'''化農曆之起日'' 
 ...
 ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
 '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  Dim KCLunar,KCLPunar , LCKNART
    KCLunar=Lunar(Syern,Smohn,Sdatn)
    KCLPunar=split(KCLunar,";")
    LSyern=Cint(KCLPunar(0))
    LSmohn=Cint(KCLPunar(1))
    LSdatn=Cint(KCLPunar(2))
    LSdlep= KCLPunar(3)

       ''' Response.write SLunar& SPLunar(3)
 ' Response.write KCLunar
       ''KLNART = KLnar(2012,1,20,"false")
       ' LCKNART = KLnar(LSyern,LSmohn,LSdatn,LSdlep)
       ' Response.write LCKNART 
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
   Dim KGLunar,KGPLunar , LKGnart
    Dim Syerng,Smohng,Sdatng,Leapg 
    Syerng=Cint(YNUM)
    Smohng=Cint(MNUM)
    Sdatng=Cint(DNUM)
   Leapg=LLeap
   ...
   'KGLunar=KLnar(LSyern,LSmohn,LSdatn,LSdlep)
 KGLunar=KLnar(Syerng,Smohng,Sdatng,Leapg)
     ' KGLunar=KLnar(Syerng,Smohng,Sdatng,"false")
       '' KGLunar=KLnar(KSyern,KSmohn,KSdatn,KSdlep)
  KGPLunar=split(KGLunar,";")
  KGyern=Cint(KGPLunar(1))
  KGmohn=Cint(KGPLunar(2))
  KGdatn=Cint(KGPLunar(3))
  KGdlep=KGPLunar(3)
   
   ''LKGnart=Tcalendar(Syerng,Smohng,Sdatng )
 LKGnart=Tcalendar(KGyern,KGmohn,KGdatn )
''''''''''''''''''''''''''''''''''''''''''''''''''''''''  
  Dim DDA3 , DDD3 ,MnD3 
  ''DT11=  #1912/2/17#
  DT113=  #1912/1/1#
 ...
 Dim TLYMDS3  
  ...
  ''  Response.write "農總日"&(TLYMDS+47)&"<br>"
   Response.write "農總日"&(TLYMDS3+47)&"<br>"
   Response.write "國總日"&DDtD3
