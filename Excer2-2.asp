...
<TABLE Border=1 BGCOLOR=#FFFF00>    
 ...
 '' Part II：輸出資料表的「內容」
  ...
  ' For i=0 to 13
  ' rls.MoveFirst		' 將目前資料錄移到第一筆
  ' While Not rls.EOF	' 判斷是否過了最後一筆
     ''If Trim(KFE(i))=Trim(rls(0)) then
  '     If Trim(RE(i))=Trim(rls(0)) then
          
    '       Row1= "<TR BGCOLOR=#00FFFF>"
    '       Response.Write Row1& RE(i) & rls(1) & "</TR>" 
    '   End If  
    '  rls.MoveNext	' 移到下一筆
  '  Wend
 ' Next
 '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''' 
  ...
  For J=1 to  6
    rls.MoveFirst		' 將目前資料錄移到第一筆
   While Not rls.EOF	' 判斷是否過了最後一筆
     If Trim(RKS(i,j))=Trim(rls(0)) then
        ''' If Trim(RE(i))=Trim(rls(0)) then
        
          Row1= "<TR BGCOLOR=#00FFFF>"
          Row1=Row1& "<FONT color=#FF0000>"&RKS(i,j)&"</FONT>"
           ' Response.Write Row1&RKS(i,j) & rls(1) & "</TR>" 
           Response.Write Row1 & rls(1) & "</TR>" 
      End If  
      rls.MoveNext	' 移到下一筆
    Wend
  Next
...
%>
 ...
 <!--<CENTER><H2><FONT color=#FF0000>若需要今流年的命 盤 評 論 表,請先在購物車(PayPal)直接購買100元台幣</FONT><HR></H2>-->
 <CENTER><H2><FONT color=#FF0000>若需要<u>今流年</u>的命盤評 論 表,請回首頁<u>留言板</u>留言</FONT><HR></H2>
 <CENTER><H2><FONT color=#FF0000>    或 Email:  tech.t1206@gmail.com 即可收到評論表,  謝謝</FONT><HR></H2>
  ...
  <!-- <CENTER><h2 align="left"><a href="http://61.222.248.199/HSU-fundb/Login-1y.asp" ><u>回流年命盤</u></a></h2>-->
