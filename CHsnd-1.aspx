...
function calElement(sYear,sMonth,sDay,week,lYear,lMonth,lDay,isLeap,cYear,cMonth,jMonth,cDay,cZod) {
  this.isToday    = false;
  this.sYear      = sYear;   
   this.sMonth     = sMonth; 
    this.sDay       = sDay;  
...
function calendar(y,m) {
 ...
 this.firstWeek = sDObj.getDay(); 
  if(m<2)  {
  Cy=GanZhi(y-1912+47); 
  Czod=Zodiac[(y-1912+47)%12]; }
 else {
  Cy=GanZhi(y-1912+48); Czod=Zodiac[(y-1912+48)%12]; }
  
  var term2=jieqi(y,2);
  var firstNode =jieqi(y,m*2);
  //document.write(term2);
 //document.write(firstNode);
 ...
 for(var i=0;i<this.length;i++) {
    if(Ld>Lx) {
       ...
       if(n==0)  firstLM = Lm;
       Ldpos[n++] = i-Ld+1;
    //  document.write(Ldpos[0]); //每月i=1日及農曆1日(Ld>Lx)之國農日差
...
var jqcd=new Array ;
var out="";
GetPureJQsinceSpring(yr,ptsa,ptsb,ptsc,jdpjq);
//for(i=0;i<12;i++){
	//jqc=jq[(2*i+21)%24]+":";
 for(i=0;i<24;i++){
	jqc=jq[(i+19)%24]+":";
	jt=Jtime(false,jdpjq[i]);
	out+=jqc+jt+"\r\n";
	jqcd[i]=jq[(i+19)%24]+":";
...
//jdpjq[0] =sjdjq[22]; jdpjq[1] =sjdjq[24];   //原此為立春 //此為驚蟄
 GetAdjustedJQ(yea, 19, 5, sjdjq);   //求出含指定年小寒開始之5個節氣JD值,以前一年的年值代入
jdpjq[0] =sjdjq[20];                     
jdpjq[1] =sjdjq[21]; 
jdpjq[2] =sjdjq[22];                     
jdpjq[3] =sjdjq[23];
...
google_ad_client = "pub-2894702992832685";
/* 728x90, 已建立 2009/3/30 */
google_ad_slot = "4608621662";
google_ad_width = 528;
google_ad_height = 90;
//-->
</script>
