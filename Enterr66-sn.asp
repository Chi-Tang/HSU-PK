...
<body  onload=initialize() background="../b01.jpg">
 ...
 SLM=TNUM
 TLD=DNUM 
 Dim  Driver , DBpath,Param , LANDA,LANY, LDBPH
 Dim conn, rsk
 ...
 ' While Not rsk.EOF	' 判斷是否過了最後一筆
  '  if (Trim(rsk(1)) =Trim(SLM) AND Trim(rsk(2)) =Trim(TLD)) then
  '    LANY = Trim(rsk(6))
  '  end if
  '   rsk.MoveNext	' 移到下一筆
 ' Wend
...
//  ExcelSheet.ActiveSheet.Cells(p2+j+1,9).Value = cldt2[j].cDay;
 ...
 //}
   // Range("A1:A11").Select;
  //  ActiveWorkbook.Names.Add Name:="寶貝", RefersToR1C1:="=Sheet1!R1C1:R11C1";
   //ExcelSheet.Range("A1:A11").CreateNames("寶貝");
  //ExcelSheet.Names ="寶貝";
  //ActiveWorkbook.Names.Add Name:="好奇"
 /// 儲存該工作表。
 //ExcelSheet.SaveAs("C:\\Inetpub\\wwwroot\\Hsu-pk\\TEST"+Nyy+Nmm+Ndd+".XLS");	
  ExcelSheet.SaveAs("C:\\Inetpub\\wwwroot\\Hsu-pk\\TEST"+SYY+(SMM+1)+(TDD+1)+".XLS");	
